// src/main/service/getTurnoReportDataModeloB.js
import fs from "fs"
import path from "path"
// Try to read machine mapping from centralized settings (fallback to defaults)
import { getGroupConfig } from "./settingsManager.js"

// Default baseDir (fallback)
const defaultBaseDir = "\\\\va\\rede\\Grupos\\Horizonte\\Departamental\\CORTE\\Alyson\\Laser\\Work\\Emma"

// Resolve baseDir from settings if available. Fall back to built-in default.
function resolveBaseDir() {
	try {
		const cfg = getGroupConfig && getGroupConfig("Emma")
		if (cfg && cfg.baseDir && typeof cfg.baseDir === "string" && cfg.baseDir.trim()) {
			console.log('[getTurnoReportDataModeloB] Usando baseDir das configurações:', cfg.baseDir)
			return cfg.baseDir
		}
	} catch (err) {
		console.warn('[getTurnoReportDataModeloB] Falha ao obter baseDir do settingsManager:', err)
	}
	console.log('[getTurnoReportDataModeloB] Usando baseDir padrão:', defaultBaseDir)
...
function resolveMachineMap() {
	try {
		...
		if (cfg && cfg.machineMap && Object.keys(cfg.machineMap).length > 0) {
			console.log('[getTurnoReportDataModeloB] Usando machineMap das configurações:', cfg.machineMap)
			return cfg.machineMap
		}
	} catch (err) {
		console.warn('[getTurnoReportDataModeloB] Falha ao obter mapeamento do settingsManager:', err)
	}
	...
	console.log('[getTurnoReportDataModeloB] Usando machineMap padrão:', defaultMap)
	return defaultMap
}

// Períodos de cada turno
// Calcula o percentual de tempo "trabalhando" (campo 5 após CUT) por período
export function getTurnoReportDataTrabalhandoModeloB(
	dateStr,
	turno = 2,
	filtroTrabalho = ["trabalhando.png"],
) {
	...
	Object.values(machineMap).forEach((machine) => {
		...
		allLines.forEach((line) => {
			...
			const tipoTrabalho = parts[4].trim()
			if (!["CUT", "PAUSAR"].includes(estado)) return
			if (!filtroTrabalho.includes(tipoTrabalho)) return
...
function getAllTxtFiles(dir) {
	try {
		if (!fs.existsSync(dir)) {
			console.warn(`[getTurnoReportDataModeloB] Diretório não existe ou inacessível: ${dir}`)
			return []
		}
		...
		console.error(`[getTurnoReportDataModeloB] Erro ao acessar diretório ${dir}:`, err.code || err.message)
		return []
	}
}
...
export function getTurnoReportData(
	...
	const todayStr = dateStr || getLocalDateString()
	console.log(
		`[DEBUG] INÍCIO getTurnoReportDataModeloB - dateStr: ${dateStr}, todayStr: ${todayStr}, turno: ${turno}, dateInicial: ${dateInicial}, dateFinal: ${dateFinal}`,
	)
...
// Lista de motivos válidos de parada para ModeloB (Emma)
const motivosValidosModeloB = [
	"Sem motivo",
	"Criando imagem",
	"Aguardando técnico",
	"Falta de operador",
...
export function getMotivosParadasPorCategoriaModeloB(dateStr, turno = null) {
	console.log(
		`[DEBUG] getMotivosParadasPorCategoriaModeloB - dateStr: ${dateStr}, turno: ${turno}`,
	)
	...
	console.log(`[DEBUG] Diretório base ModeloB: ${baseDir}`)
	console.log(
		`[DEBUG] Processando ${Object.keys(machineMap).length} máquinas ModeloB`,
	)

	Object.values(machineMap).forEach((machine) => {
		console.log(`[DEBUG] ========================================`)
		console.log(`[DEBUG] Processando máquina ModeloB: ${machine}`)
		const dir = path.join(baseDir, machine)
		console.log(`[DEBUG] Caminho completo ModeloB: ${dir}`)
		const txtFiles = getAllTxtFiles(dir)
		if (!txtFiles.length) {
		...
		allLines.forEach((line) => {
			...
			if (["OCUPAÇÃO", "CUT", "WORKING", "PAUSAR"].includes(cat)) return // Exclui estados de trabalho
			...
			if (!motivosValidosModeloB.includes(motivoLimpo)) {
				console.log(
					`[DEBUG] ${machine} - Motivo "${motivoLimpo}" não está na lista de motivos válidos ModeloB`,
				)
				return
			}
	...
	console.log(`[DEBUG] ========================================`)
	console.log(`[DEBUG] RESUMO FINAL ModeloB:`)
	...
	console.log(`[DEBUG] Resultado final ModeloB:`, result)
	return result
}

export default getTurnoReportData
