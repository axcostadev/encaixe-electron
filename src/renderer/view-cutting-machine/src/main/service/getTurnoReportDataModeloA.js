...
const defaultBaseDir = "\\\\va\\rede\\Grupos\\Horizonte\\Departamental\\CORTE\\Alyson\\Laser\\Work\\Lectra"

// Resolve baseDir from settings if available. Fall back to built-in default.
function resolveBaseDir() {
	try {
		const cfg = getGroupConfig && getGroupConfig("Lectra")
		if (cfg && cfg.baseDir && typeof cfg.baseDir === "string" && cfg.baseDir.trim()) {
			console.log('[getTurnoReportDataModeloA] Usando baseDir das configurações:', cfg.baseDir)
			return cfg.baseDir
		}
	} catch (err) {
		console.warn('[getTurnoReportDataModeloA] Falha ao obter baseDir do settingsManager:', err)
	}
	console.log('[getTurnoReportDataModeloA] Usando baseDir padrão:', defaultBaseDir)
...
function resolveMachineMap() {
	try {
		...
		if (cfg && cfg.machineMap && Object.keys(cfg.machineMap).length > 0) {
			console.log('[getTurnoReportDataModeloA] Usando machineMap das configurações:', cfg.machineMap)
			return cfg.machineMap
		}
	} catch (err) {
		console.warn('[getTurnoReportDataModeloA] Falha ao obter mapeamento do settingsManager:', err)
	}
...
function getAllTxtFiles(dir) {
	try {
		if (!fs.existsSync(dir)) {
			console.warn(`[getTurnoReportDataModeloA] Diretório não existe ou inacessível: ${dir}`)
			return []
		}
		...
		console.error(`[getTurnoReportDataModeloA] Erro ao acessar diretório ${dir}:`, err.code || err.message)
		return []
	}
}
...
export function getTurnoReportData(
	...
	console.log(
		`[DEBUG] INÍCIO getTurnoReportDataModeloA - dateStr: ${dateStr}, todayStr: ${todayStr}, turno: ${turno}, dateInicial: ${dateInicial}, dateFinal: ${dateFinal}`,
	)
	...
	Object.values(machineMap).forEach((machine) => {
		...
		txtFiles.forEach((filePath) => {
			try {
				...
				if (error.code === "EPERM") {
					console.warn(
						`[WARN][${machine}] Sem permissão para ler arquivo ${filePath} - continuando...`,
					)
				} else {
					console.error(
						`[ERROR][${machine}] Erro ao ler arquivo ${filePath}:`,
						error.message,
					)
				}
			}
...
// Lista de motivos válidos de parada para ModeloA (Lectra)
const motivosValidosModeloA = [
	"Sem motivo",
	"Criando imagem",
	"Aguardando técnico",
	"Falta de operador",
...
export function getMotivosParadasPorCategoriaModeloA(dateStr, turno = null) {
	console.log(
		`[DEBUG] getMotivosParadasPorCategoriaModeloA - dateStr: ${dateStr}, turno: ${turno}`,
	)
	...
	console.log(`[DEBUG] Diretório base ModeloA: ${baseDir}`)
	console.log(
		`[DEBUG] Processando ${Object.keys(machineMap).length} máquinas ModeloA`,
	)

	Object.values(machineMap).forEach((machine) => {
		console.log(`[DEBUG] ========================================`)
		console.log(`[DEBUG] Processando máquina ModeloA: ${machine}`)
		const dir = path.join(baseDir, machine)
		console.log(`[DEBUG] Caminho completo ModeloA: ${dir}`)
		const txtFiles = getAllTxtFiles(dir)
		if (!txtFiles.length) {
		...
		allLines.forEach((line) => {
			...
			if (!motivosValidosModeloA.includes(motivoLimpo)) {
				console.log(
					`[DEBUG] ${machine} - Motivo "${motivoLimpo}" não está na lista de motivos válidos ModeloA`,
				)
				return
			}
	...
	console.log(`[DEBUG] ========================================`)
	console.log(`[DEBUG] RESUMO FINAL ModeloA:`)
	...
	console.log(`[DEBUG] Resultado final ModeloA:`, result)
	return result
}
