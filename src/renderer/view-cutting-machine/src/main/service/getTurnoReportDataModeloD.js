// src/main/service/getTurnoReportDataModeloD.js — Comelz Montagem
import fs from "fs"
...
function resolveBaseDir() {
	try {
		const cfg = getGroupConfig && getGroupConfig("ComelzMontagem")
		if (cfg && cfg.baseDir && typeof cfg.baseDir === "string" && cfg.baseDir.trim()) {
			console.log('[getTurnoReportDataModeloD] Usando baseDir das configurações:', cfg.baseDir)
			return cfg.baseDir
		}
	} catch (err) {
		console.warn('[getTurnoReportDataModeloD] Falha ao obter baseDir do settingsManager:', err)
	}
	console.log('[getTurnoReportDataModeloD] Usando baseDir padrão:', defaultBaseDir)
...
function resolveMachineMap() {
	try {
		...
		if (cfg && cfg.machineMap && Object.keys(cfg.machineMap).length > 0) {
			console.log('[getTurnoReportDataModeloD] Usando machineMap das configurações:', cfg.machineMap)
			return cfg.machineMap
		}
	} catch (err) {
		console.warn('[getTurnoReportDataModeloD] Falha ao obter mapeamento do settingsManager:', err)
	}
	...
	console.log('[getTurnoReportDataModeloD] Usando machineMap padrão:', defaultMap)
	return defaultMap
}
...
function getAllTxtFiles(dir) {
	try {
		if (!fs.existsSync(dir)) {
			console.warn(`[getTurnoReportDataModeloD] Diretório não existe ou inacessível: ${dir}`)
			return []
		}
		...
		console.error(`[getTurnoReportDataModeloD] Erro ao acessar diretório ${dir}:`, err.code || err.message)
		return []
	}
}
...
export function getTurnoReportDataModeloD(
	dateStr,
	turno = 2,
	dateInicial = null,
	dateFinal = null,
	...
	console.log(
		`[DEBUG] INÍCIO getTurnoReportDataModeloD - dateStr: ${dateStr}, todayStr: ${todayStr}, turno: ${turno}, dateInicial: ${dateInicial}, dateFinal: ${dateFinal}`,
	)
...
export function getMotivosParadasPorCategoriaModeloD(dateStr, turno = null) {
	console.log(
		`[DEBUG] getMotivosParadasPorCategoriaModeloD - dateStr: ${dateStr}, turno: ${turno}`,
	)
	...
	Object.values(machineMap).forEach((machine) => {
		...
		txtFiles.forEach((filePath) => {
			try {
				...
				console.error(`[getTurnoReportDataModeloD] Erro ao ler arquivo ${filePath}:`, error)
			}
		})
		...
		allLines.forEach((line) => {
			...
			if (!motivosValidosModeloD.includes(motivoLimpo)) return
