...
async function salvarDadosNoBanco(
	...
	try {
		console.log("[SALVAMENTO EMMA] Salvando dados no banco:", { date, turno })
		...
		console.log(
			`[SALVAMENTO EMMA] ${dadosParaSalvar.length} registros salvos com sucesso`,
		)
	} catch (error) {
		console.error("[SALVAMENTO EMMA] Erro ao salvar dados:", error)
	}
}
...
export default function TurnoReport() {
		...
		async function loadSettings() {
			try {
				...
				if (res && res.success && res.settings) {
					...
					// Preferir o mapeamento do grupo Emma quando disponível
					const emma = groups?.Emma?.machineMap ?? groups
					if (emma && Object.keys(emma).length > 0) {
						setMachineMapEdit(emma)
					}
				}
			} catch (err) {
				console.warn('[TurnoReportModeloB] Falha ao carregar settings:', err)
			}
		}
	...
	// Estado para controlar se mostra trabalhando
	const [mostrarTrabalhando, setMostrarTrabalhando] = useState(true)
	...
	>>(null)
	const [_trabalhando, setTrabalhando] = useState<null | Record<
		string,
		Record<string, number>
	>>(null)
	const [geralTrabalhando, setGeralTrabalhando] = useState<number | null>(null)
	...
	useEffect(() => {
		if (turnoSelecionado !== null) {
			...
			window.electron?.ipcRenderer
				...
				.then((data) => {
					console.log("[FRONTEND Emma] OCUPACAO RECEBIDA:", data)
				...
				.catch((error) => {
					console.error("[FRONTEND Emma] ERRO OCUPAÇÃO:", error)
					setOcupacao(null)
				})

			// Buscar dados de trabalhando
			setLoadingMessage("Carregando dados de trabalho...")
			window.electron?.ipcRenderer
				?.invoke("turno-report-data-trabalhando-emma", {
					dateInicial: dataInicial,
					dateFinal: dataFinal,
					date: dataSelecionada,
					turno: turnoSelecionado,
				...
				.then((data) => {
					console.log("[FRONTEND Emma] TRABALHANDO RECEBIDA:", data)
					setLoadingProgress(66)
					setLoadingMessage("Dados de trabalho carregados...")
					
					if (data && typeof data === "object") {
						setTrabalhando(data)

						// Calcular geral trabalhando usando a mesma lógica do getGeralTurnoTotal()
						let totalValues: number[] = []

						// Para cada máquina não ignorada
						maquinas.forEach((_, i) => {
							if (maquinasIgnoradas.includes(i)) return
							...
							if (machineData) {
								...
								if (values.length > 0) {
									const avg =
										values.reduce((a, b) => a + (b as number), 0) /
										values.length
									totalValues.push(avg)
								}
							}
						})
						...
						if (totalValues.length > 0) {
							...
							setGeralTrabalhando(Math.round(media))
							console.log(
								"[FRONTEND Emma] GERAL TRABALHANDO CALCULADO:",
								Math.round(media),
							)
						} else {
							setGeralTrabalhando(null)
							console.log("[FRONTEND Emma] GERAL TRABALHANDO: NULL (sem dados)")
						}
					} else {
						setTrabalhando(null)
						setGeralTrabalhando(null)
				...
				.catch((error) => {
					console.error("[FRONTEND Emma] ERRO TRABALHANDO:", error)
					setTrabalhando(null)
					setGeralTrabalhando(null)
					setIsLoading(false)
				})
	...
	useEffect(() => {
		if (ocupacao && turnoSelecionado !== null) {
			fetchMotivosDataModeloB(dataSelecionada, turnoSelecionado)
				.then((motivosRawData) => {
					if (motivosRawData) {
	...
	const captureData = React.useCallback(async () => {
		...
		try {
			...
			console.log(
				`[EMMA-CAPTURE] Capturando dados para turno ${turnoSelecionado + 1} na data ${dataSelecionada}`,
			)

			// Enviar dados para o BANCO DE DADOS
			await ipcHelper.invoke("capture-turno-data", {
				date: dataSelecionada,
			...
			console.log("[EMMA-CAPTURE] Dados capturados com sucesso!")
		} catch (error) {
			console.error("[EMMA-CAPTURE] Erro ao capturar dados:", error)
		}
	...
	useEffect(() => {
		if (ocupacao && turnoSelecionado !== null && dataSelecionada) {
			console.log(
				"[EMMA-CAPTURE] Dados carregados, iniciando captura automática...",
			)
			captureData()
		}
		...
		<div className="p-6 bg-white rounded-lg shadow-lg">
			...
			<div className="flex justify-between items-center mb-4">
				<h2 className="text-xl font-bold text-blue-700">
					CORTE - EMMA - RELATÓRIO % OCUPAÇÃO DE MÁQUINAS
				</h2>
				<div className="flex items-end gap-4">
					{/* Botão para mostrar/ocultar trabalhando */}
					<div className="flex flex-col items-start">
						<span className="text-gray-600 text-xs opacity-0">.</span>
						<button
							onClick={() => setMostrarTrabalhando(!mostrarTrabalhando)}
			...
			{/* Controles em uma linha - apenas máquinas ignoradas agora */}
				...
				<div className="bg-gray-50 rounded p-4">
					<h3 className="font-semibold mb-2">
						{turnoSelecionado === null
							...
							: `EMMA - ${turnos[turnoSelecionado]} - MÁQUINAS`}
					</h3>
					...
					<div className="bg-gray-50 rounded p-4 flex flex-col items-center">
						...
						<div className="flex flex-col items-center gap-4">
							<CategoriaDonutChart
								...
								trabalhando={mostrarTrabalhando ? geralTrabalhando : null}
