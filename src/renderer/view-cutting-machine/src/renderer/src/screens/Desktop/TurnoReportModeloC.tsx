...
async function salvarDadosNoBanco(
	...
	try {
		console.log("[SALVAMENTO COMELZ] Salvando dados no banco:", { date, turno })
		...
		console.log(
			`[SALVAMENTO COMELZ] ${dadosParaSalvar.length} registros salvos com sucesso`,
		)
	} catch (error) {
		console.error("[SALVAMENTO COMELZ] Erro ao salvar dados:", error)
	}
}
...
export default function TurnoReportModeloC() {
		...
		async function loadSettings() {
			try {
				...
				if (res && res.success && res.settings) {
					...
					// Para Comelz, buscar especificamente o mapeamento do Comelz
					let mapping = null
					if (groups && typeof groups === "object") {
						const gAny = groups as any
						// Busca APENAS o mapeamento do Comelz
						mapping = gAny.Comelz?.machineMap ?? null
					}
					// Se não encontrou nas configurações, usa o padrão já definido (não sobrescreve)
					if (mapping && Object.keys(mapping).length > 0) {
						console.log('[TurnoReportModeloC] Carregando machineMap do Comelz:', mapping)
						setMachineMapEdit(mapping)
					} else {
						console.log('[TurnoReportModeloC] Usando machineMap padrão do Comelz')
					}
				}
			} catch (err) {
				console.warn('[TurnoReportModeloC] Falha ao carregar settings:', err)
			}
		}
	...
	useEffect(() => {
		if (turnoSelecionado !== null) {
			...
			// Carrega dados de ocupação usando handler específico do Comelz
			window.electron?.ipcRenderer
				?.invoke("turno-report-data-comelz", {
				...
				.then((data) => {
					...
					// Usar o mesmo índice de turno (0,1,2) que o backend e consultas esperam
	...
	useEffect(() => {
		if (ocupacao && turnoSelecionado !== null) {
			fetchMotivosDataModeloC(dataSelecionada, turnoSelecionado)
				.then((motivosRawData) => {
					if (motivosRawData) {
		...
		<div className="p-6 bg-white rounded-lg shadow-lg">
			...
			<div className="flex justify-between items-center mb-4">
				<h2 className="text-xl font-bold text-blue-700">
					CORTE - COMELZ - RELATÓRIO % OCUPAÇÃO DE MÁQUINAS
				</h2>
				...
				<div className="bg-gray-50 rounded p-4">
					<h3 className="font-semibold mb-2">
						{turnoSelecionado === null
							...
							: `COMELZ - ${turnos[turnoSelecionado]} - MÁQUINAS`}
					</h3>
