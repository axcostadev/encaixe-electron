...
// Exemplo de dados estáticos para o layout - Comelz Montagem
const turnos = ["1º Turno", "2º Turno", "3º Turno"]

// Mapeamento padrão para os nomes das máquinas usados no backend - Comelz Montagem
...
async function salvarDadosNoBanco(
	...
	try {
		console.log("[SALVAMENTO COMELZ MONTAGEM] Salvando dados no banco:", { date, turno })
		...
		console.log(
			`[SALVAMENTO COMELZ MONTAGEM] ${dadosParaSalvar.length} registros salvos com sucesso`,
		)
	} catch (error) {
		console.error("[SALVAMENTO COMELZ MONTAGEM] Erro ao salvar dados:", error)
	}
}

export default function TurnoReportModeloD() {
		...
		async function loadSettings() {
			try {
				...
				if (res && res.success && res.settings) {
					// Pode existir um objeto machineGroups com vários grupos
					const groups = res.settings.machineGroups ?? {}
					let mapping = null
					if (groups && typeof groups === "object") {
						const gAny = groups as any
						// Busca APENAS o mapeamento do ComelzMontagem
						mapping = gAny.ComelzMontagem?.machineMap ?? null
					}
					// Se não encontrou nas configurações, usa o padrão já definido (não sobrescreve)
					if (mapping && Object.keys(mapping).length > 0) {
						console.log('[TurnoReportModeloD] Carregando machineMap do ComelzMontagem:', mapping)
						setMachineMapEdit(mapping)
					} else {
						console.log('[TurnoReportModeloD] Usando machineMap padrão do ComelzMontagem')
					}
				}
			} catch (err) {
				console.warn('[TurnoReportModeloD] Falha ao carregar settings:', err)
			}
		}

		loadSettings()

		// Também escuta notificações locais que o Setup dispara após salvar
	...
	useEffect(() => {
		if (turnoSelecionado !== null) {
			...
			// Carrega dados de ocupação usando handler específico do Comelz Montagem
			window.electron?.ipcRenderer
				?.invoke("turno-report-data-comelz-montagem", {
				...
				.then((data) => {
					...
					salvarDadosNoBanco(data, dataSelecionada, turnoSelecionado)
				...
				.catch((error) => {
					console.error("[FRONTEND] Erro ao carregar ocupação:", error)
					setOcupacao(null)
					setIsLoading(false)
				})
		}
	...
	useEffect(() => {
		if (ocupacao && turnoSelecionado !== null) {
			fetchMotivosDataModeloD(dataSelecionada, turnoSelecionado)
				.then((motivosRawData) => {
					if (motivosRawData) {
		...
		<div className="p-6 bg-white rounded-lg shadow-lg">
			...
			<div className="flex justify-between items-center mb-4">
				<h2 className="text-xl font-bold text-blue-700">
					CORTE - COMELZ MONTAGEM - RELATÓRIO % OCUPAÇÃO DE MÁQUINAS
				</h2>
				...
				<div className="bg-gray-50 rounded p-4">
					<h3 className="font-semibold mb-2">
						{turnoSelecionado === null
							...
							: `COMELZ MONTAGEM - ${turnos[turnoSelecionado]} - MÁQUINAS`}
					</h3>
