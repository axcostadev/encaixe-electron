...
// Exemplo de dados estáticos para o layout - Comelz Solas
const turnos = ["1º Turno", "2º Turno", "3º Turno"]

// Mapeamento padrão para os nomes das máquinas usados no backend - Comelz Solas
...
async function salvarDadosNoBanco(
	...
	try {
		console.log("[SALVAMENTO COMELZ SOLAS] Salvando dados no banco:", { date, turno })
		...
		console.log(
			`[SALVAMENTO COMELZ SOLAS] ${dadosParaSalvar.length} registros salvos com sucesso`,
		)
	} catch (error) {
		console.error("[SALVAMENTO COMELZ SOLAS] Erro ao salvar dados:", error)
	}
}

export default function TurnoReportModeloE() {
			...
			try {
				...
				if (res && res.success && res.settings) {
					...
					if (groups && typeof groups === "object") {
						...
						// Busca APENAS o mapeamento do ComelzSolas
						mapping = gAny.ComelzSolas?.machineMap ?? null
					}
					// Se não encontrou nas configurações, usa o padrão já definido (não sobrescreve)
					if (mapping && Object.keys(mapping).length > 0) {
						console.log('[TurnoReportModeloE] Carregando machineMap do ComelzSolas:', mapping)
						setMachineMapEdit(mapping)
					} else {
						console.log('[TurnoReportModeloE] Usando machineMap padrão do ComelzSolas')
					}
				}
			} catch (err) {
				console.warn('[TurnoReportModeloE] Falha ao carregar settings:', err)
			}
		}
	...
	useEffect(() => {
		if (turnoSelecionado !== null) {
			...
			// Carrega dados de ocupação usando handler específico do Comelz Solas
			window.electron?.ipcRenderer
				?.invoke("turno-report-data-comelz-solas", {
	...
	useEffect(() => {
		if (ocupacao && turnoSelecionado !== null) {
			fetchMotivosDataModeloE(dataSelecionada, turnoSelecionado)
				.then((motivosRawData) => {
					if (motivosRawData) {
		...
		<div className="p-6 bg-white rounded-lg shadow-lg">
			...
			<div className="flex justify-between items-center mb-4">
				<h2 className="text-xl font-bold text-blue-700">
					CORTE - COMELZ SOLAS - RELATÓRIO % OCUPAÇÃO DE MÁQUINAS
				</h2>
				...
				<div className="bg-gray-50 rounded p-4">
					<h3 className="font-semibold mb-2">
						{turnoSelecionado === null
							...
							: `COMELZ SOLAS - ${turnos[turnoSelecionado]} - MÁQUINAS`}
					</h3>
