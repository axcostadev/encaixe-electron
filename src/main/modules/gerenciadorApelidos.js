...
export async function getAllApelidos() {
  if (dbConn) {
    return new Promise((resolve, reject) => {
      dbConn.all('SELECT componente, apelido FROM apelidos ORDER BY componente', [], (err, rows) => {
        if (err) return reject(err)
        const mapa = {}
...
export async function removeApelido(componente) {
  ...
  if (dbConn) {
    return new Promise((resolve, reject) => {
      dbConn.run('DELETE FROM apelidos WHERE componente = ?', [key], function (err) {
        if (err) return reject(err)
        getAllApelidos().then(resolve).catch(reject)
      })
    })
...
export async function getApelido(componente) {
  ...
  if (dbConn) {
    return new Promise((resolve, reject) => {
      dbConn.get('SELECT apelido FROM apelidos WHERE componente = ?', [key], (err, row) => {
        if (err) return reject(err)
